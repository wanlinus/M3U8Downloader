...
- **读结果集一律按列名**（`SqliteRow`），别写序号（`reader.GetString(0)`）。
  序号写法在表中间插一列时会**静默读错列** —— 不报错、不崩，只是数据悄悄串了，
  编译期更看不出来；按列名则是列名写错当场抛异常。
- **没引 ORM**：EF Core 对"一行设置 + 几个任务 + 几十条历史"这个数据量太重
  （会让 Core 从 1 个包变成 6 个），Dapper 是更合适的档位但暂时也没必要 ——
  手写映射的痛点已经被 `SqliteRow` 解决掉了。
...
- **托盘常驻的应用要把 `DispatcherShutdownMode` 设成 `OnExplicitShutdown`**。`Application.Start`
  默认给的是 `OnLastWindowClose`（"这个线程上的 XAML 窗口全关了就收摊"），而本程序的设计是
  主窗口缩进托盘只做隐藏、后台继续下载 —— 默认规则对它是错的。改成显式退出之后，
  **托盘菜单的「退出」必须自己调 `Exit()`**，否则会留下一个关不掉的进程。
- **多开的窗口要留住托管引用**（本项目的播放窗口存在 `App.OpenPlayers` 里）。WinUI 的 `Window`
  不会自己保活，没有引用的实例会被 GC 回收、窗口凭空消失；引用字典的键用 **打开时就取好的**
  `WindowId`，别在 `Closed` 里回头访问已经关掉的窗口的 `AppWindow`。

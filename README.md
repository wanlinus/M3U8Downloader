...
每一集下载时都使用**自己专属的暂存目录**，本集的分片与清单都放在里面：

```
%TEMP%\M3U8Downloader\series\30450_1_0007\
├── manifest.json        # 清单：播放列表地址、分片总数、总时长、跳过项、指纹、产物路径
├── seg_000000.ts        # 分片
├── seg_000001.ts
└── ...
```

流程：**下载 → 合并成一个整体视频文件 → 全量校验 → 全部通过才删除暂存目录。**

> **中间任何一步出错都不删。** 失败时暂存目录连同分片一并保留，便于排查或下次续传。

### 为什么清单里要存"指纹"

源站可能**每次请求都重新生成播放列表**（典型做法是随机插播广告），于是
「分片序号 ↔ 内容」的对应关系在不同次请求之间会变。若续传时直接复用上一次的残留分片，

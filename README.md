...
| **苹果 CMS** | MacCMS v10 及其衍生模板，国内占比最高 | 已实测多个不同模板 |
...
| 站点解析（列剧集） | ⚠️ 仅当适配器声明需要 | 绝大多数站点直连；少数挂在 Cloudflare 后面的站点（如努努影院）国内直连会被重置，这时才走代理 |
| 直链可用性探测 | ⚠️ 先直连、失败才用代理 | 探测的是分片 CDN，实测直连即可，不该白耗代理流量 |
| **下载 FFmpeg** | ✅ 需要时走代理 | 唯一要访问 GitHub 的环节，国内直连通常不通 |

- 站点是否走代理由**适配器自己声明**（`ISiteAdapter.NeedsProxy`），
  不是全局开关 —— 不会因为开了代理就让所有站点都绕一圈；
- 只认「设置里填的代理地址」，**不看**「下载 FFmpeg 时使用代理」那个勾：
  填了地址就说明这台机器上有代理可用；
...
> 声明 `NeedsProxy` 的站点，如果没填代理地址，解析时会直接告诉你

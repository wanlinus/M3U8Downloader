...
| 适配器 | 覆盖的站点形态 | 状态 |
|---|---|---|
| **努努影院** | nnyy.in 自研站点（`/{栏目}/{剧ID}.html`） | 已实测：17 集解析、单集 951 分片下完并通过全部校验 |
...
| 直链 | `GET /_gp/{剧ID}/{ep_slug}` → `{"video_plays":[{"play_data":"…index.m3u8","src_site":"bfzy"}]}` |

两个实测出来的关键点：

- **同一部剧的不同集，可用源并不相同**（《交锋》第 1 集 9 个源、第 17 集只剩 1 个），
  所以直链必须逐集现取，不能一次解析全集；
- 列表里靠前的源**可能已经失效**（返回 HTML 而不是清单），所以会按顺序探测前几个源，
  挑一个真能取到 `#EXTM3U` 的再下载。

> 该站点挂在 Cloudflare 后面，国内直连会被重置（`curl: (35) Recv failure`），
> 因此它的适配器声明了 `NeedsProxy`：**页面解析走代理，分片下载仍直连**

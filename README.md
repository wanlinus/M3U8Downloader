...
| 适配器 | 覆盖的形态 | 实测 |
|---|---|---|
| **苹果 CMS** | MacCMS v10 及其衍生模板，国内占比最高；三种 URL 形态都认 | 多个模板；欧乐影院（原生形态）、影迷界影院（斜杠形态 + 详情页/播放页两套编号） |
| **努努影院** | nnyy.in 自研站点（`ep_slug` + `/_gp/` 接口） | 9 个播放源 / 18 集；单集 951 分片下完并全部校验通过 |
| **通用解析（兜底）** | 页面里直接写着 m3u8 地址的站点 | 专用适配器都不认时自动出手 |

三点值得说明：

- **不写死 URL 前缀、不依赖 CSS 类名**。苹果 CMS 的播放页形态三种都认：
  伪静态 `/vodplay/30450-1-1.html`、原生 `/index.php/vod/play/id/83912/sid/1/nid/1.html`、
  斜杠 `/play/2337178967/7/1.html`；剧集列表靠"页面里指向同一剧的链接"来收集，换模板也能用。
  有些站点的详情页 ID 与播放页 ID 是**两套编号**（影迷界影院：`/t/62329.html` ↔
  `/play/2337178967/…`），所以是**先收集链接、再投票选出本剧 ID**，而不是拿详情页 ID 硬过滤。
...
要加新站点：实现 `ISiteAdapter` —— `CanHandle` / `ParseAsync` / `ResolvePlaylistUrlAsync`
这三个是必须的，`Kind` / `Name` / `Priority` / `NeedsProxy` 按需重写，
...
| J | 站点适配层：自动登记、努努（含电影页）、通用兜底、空页面报错、影迷界影院（斜杠形态 / 两套编号） |
| K | 直连不得走系统代理：把进程默认代理指到黑洞，站点解析与分片清单仍须直连成功 |
...
- **直连就是真直连**：直连客户端显式关掉了代理，**不会**去用系统（Windows 设置里的）代理。
  你开着 Clash 之类的工具时，国内站点照样一分代理流量都不花。
- **回退按主机记忆**：记住之后同一主机的后续请求直接走代理。
  不记的话每个分片、每次页面请求都要先白等一遍超时 —— 实测识别欧乐影院
  从 36 秒降到 9 秒，靠的就是这一条。
- 404/410 这类「资源真没了」**不会**触发代理（换了也没用）；
- 连接超时 5 秒，不用干等；万一代理那条路也走不通，会用一个连接超时放宽到 30 秒的
  直连客户端再试一次 —— 慢不等于不通；
